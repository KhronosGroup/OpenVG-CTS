 ...
 * VG_PAINT_PATTERN_TILING_MODE = VG_TILE_REFLECT.
 *-------------------------------------------------------------------*/
CT_Result I40104_PaintPatternImage_by_HYBRID(CT_File *AnsFile)
{
    VGImage checker;
    VGPaint paint;
    VGPath	path;
	...
	/* Create the paint and set parameters for the pattern reflect */
    paint = vgCreatePaint();
	...
	/*---------------------------------------------------------------------
	 * Translate the image out of bounds and scale it to half the size.
	 *-------------------------------------------------------------------*/
    vgSeti(VG_MATRIX_MODE, VG_MATRIX_FILL_PAINT_TO_USER);
    vgLoadIdentity();
	vgTranslate(1050, 230);
	vgScale(0.5, 0.5);
