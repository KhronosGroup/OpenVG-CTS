...
CT_Result G30237_PathStroking_by_ATI (CT_File *AnsFile)
{
    VGPaint fillpaint;
    VGPaint strokepaint;

 VGPath path170;
 VGubyte  path170cmd[]={ /* 2 segments */
   VG_MOVE_TO_ABS    , VG_CUBIC_TO_ABS   };
 VGfloat path170coord[]={ /* 8 coordinates */
    ...
    path170=vgCreatePath(VG_PATH_FORMAT_STANDARD,VG_PATH_DATATYPE_F, 1.0000f,0.0000f, 0,0, (unsigned int)VG_PATH_CAPABILITY_APPEND_TO);
    vgAppendPathData(path170,sizeof(path170cmd)/sizeof(VGubyte),path170cmd,path170coord);

    if ( path170 == VG_INVALID_HANDLE )
        return CT_ERROR;
    ...
    vgDrawPath(path170,VG_STROKE_PATH);
    vgDestroyPath(path170);
