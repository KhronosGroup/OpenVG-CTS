...
CT_Result N10201_MSAA_MASK_by_HUONE(CT_File *AnsFile, VGuint subtest)
{
    VGPath path1, path2, pathCover;
    VGPaint paint;
    VGfloat errorSum[2] = { 0.0f, };
    VGubyte *buf1 = NULL, *buf2 = NULL;
    ...
    VGubyte cmdsCover[] = { VG_MOVE_TO_ABS, VG_LINE_TO_ABS, VG_LINE_TO_ABS, VG_LINE_TO_ABS, VG_CLOSE_PATH };
    VGfloat coordsCover[] = { 0.0f,0.0f, 64.0f,0.0f, 64.0f,64.0f, 0.0f,64.0f };
    ...
    pathCover = vgCreatePath(VG_PATH_FORMAT_STANDARD, VG_PATH_DATATYPE_F, 1, 0, 0, 0, VG_PATH_CAPABILITY_ALL );
    if ( pathCover== VG_INVALID_HANDLE ) {
        vgDestroyPath( path1 );
        vgDestroyPath( path2 );
        return CT_ERROR;
    }
    ...
    switch ( subtest % 3 )
    {
        /* Lozenge */
        case 0:
            vgAppendPathData( path1, 5, cmds1, coords1 );
            vgAppendPathData( path2, 10, cmds2, coords2 );
            vgAppendPathData( pathCover, 5, cmdsCover, coordsCover );
            break;
        /* Circle */
        case 1:
            vgAppendPathData( path1, 4, cmds1, coords1 );
            vgAppendPathData( path2, 9, cmds2, coords2 );
            vgAppendPathData( pathCover, 5, cmdsCover, coordsCover );
            break;
        /* Arc + Triangle */
        case 2:
    ...
    /* First Image Save */
    buf1 = (VGubyte *)MALLOC(WINDSIZEX * WINDSIZEY * 4 * sizeof(VGubyte));
    if ( !buf1 ) {
        vgDestroyPath( path1 );
        vgDestroyPath( path2 );  
        vgDestroyPath( pathCover );  
    ...
    vgRenderToMask( path2, VG_FILL_PATH, VG_SET_MASK );

    {
        /* 6. Draw Full Screen to Red Color */
        vgLoadIdentity();
        vgDrawPath( pathCover, VG_FILL_PATH );
    }

    {
        VGint i;
        
        buf2 = (VGubyte *)MALLOC(WINDSIZEX * WINDSIZEY * 4 * sizeof(VGubyte));
        if ( !buf2 ) {
            vgDestroyPath( path1 );
            vgDestroyPath( path2 );  
            vgDestroyPath( pathCover );  
        ...
        vgReadPixels(buf2, WINDSIZEX*4, VG_sRGBA_8888, 0, 0, WINDSIZEX, WINDSIZEY);

        /* Image Compare. must be identical */
        for ( i = 0; i < WINDSIZEX * WINDSIZEY; i++ ) {
            if ( buf1[i*4+0] != buf2[i*4+0] ||
                 buf1[i*4+1] != buf2[i*4+1] ||
                 buf1[i*4+2] != buf2[i*4+2] || 
                 buf1[i*4+3] != buf2[i*4+3] ) {  
                errorSum[0]++;
            }
        }
    ...
    if ( vgGetError() == VG_NO_ERROR )
        return SaveValueFile(2, errorSum, AnsFile);
    else
        return CT_ERROR;
}
