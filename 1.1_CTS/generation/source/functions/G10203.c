...
CT_Result G10203_PathSegment_by_HYBRID(CT_File *AnsFile)
{
    VGPaint stroke;
    VGPath spiral;
    VGPath loop;
...
/*  Tests that quadric tesselation can keep up
 *	with different curvatures by rendering a spiral.
 */
...
/*  This is just a bit over 1 and 1/2 pixels on
 *	256 X 256 display. Should prevent fading due antialiasing.
 */
    /*vgSetf(VG_STROKE_LINE_WIDTH, 0.0065f);*/
 ...
 */
	...
	startCmds[0] = VG_MOVE_TO;
	startCmds[1] = VG_LINE_TO_ABS;
    ...
    {
        VGubyte temp[4] = {VG_QUAD_TO_ABS, VG_QUAD_TO_ABS, VG_QUAD_TO_ABS, VG_QUAD_TO_ABS};
        for (i = 0; i < 4; i++)
        {
            spiralCmds[i] = temp[i];
        }
    ...
    {
        VGfloat temp[16] = {0.91f, 0.05f, 0.91f, 0.5f, 0.91f, 0.89f, 0.5f, 0.89f,
							0.14f, 0.89f, 0.14f, 0.5f, 0.14f, 0.17f, 0.5f, 0.17f};
        for (i = 0; i < 16; i++)
        {
