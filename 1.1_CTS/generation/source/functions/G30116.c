...
CT_Result G30116_PathStroking_by_ATI (CT_File *AnsFile)
{
    VGPaint fillpaint;
    VGPaint strokepaint;
 ...
 VGfloat path223coord[]={ /* 6 coordinates */
    ...
    path222=vgCreatePath(VG_PATH_FORMAT_STANDARD,VG_PATH_DATATYPE_F, 1.0000f,0.0000f, 0,0, (unsigned int)VG_PATH_CAPABILITY_APPEND_TO);
    vgAppendPathData(path222,sizeof(path222cmd)/sizeof(VGubyte),path222cmd,path222coord);
    path223=vgCreatePath(VG_PATH_FORMAT_STANDARD,VG_PATH_DATATYPE_F, 1.0000f,0.0000f, 0,0, (unsigned int)VG_PATH_CAPABILITY_APPEND_TO);
    vgAppendPathData(path223,sizeof(path223cmd)/sizeof(VGubyte),path223cmd,path223coord);

    if ( path222 == VG_INVALID_HANDLE )
        return CT_ERROR;
    if ( path223 == VG_INVALID_HANDLE )
        return CT_ERROR;
  ...
  vgDrawPath(path222,VG_FILL_PATH|VG_STROKE_PATH);
  ...
  vgSeti(VG_STROKE_CAP_STYLE,VG_CAP_BUTT  );
  vgSeti(VG_STROKE_JOIN_STYLE,VG_JOIN_MITER);
  vgDrawPath(path223,VG_FILL_PATH|VG_STROKE_PATH);

    vgDestroyPath(path222);
    vgDestroyPath(path223);
