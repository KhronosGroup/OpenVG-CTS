...
CT_Result G30302_PathStroking_by_ATI (CT_File *AnsFile)
{
    VGPaint fillpaint;
    VGPaint strokepaint;

 VGPath path161;
 VGubyte  path161cmd[]={ /* 2 segments */
   VG_MOVE_TO_ABS    , VG_SCWARC_TO_ABS  };
 VGfloat path161coord[]={ /* 7 coordinates */
     2.000000e+000f,  -2.000000e+000f,   2.828427e+000f,   2.828427e+000f,   0.000000e+000f,  -2.000000e+000f,
    -2.000000e+000f};

    VGfloat matrix[9];
    VGfloat defaultColor[] = {1.0f, 1.0f, 1.0f, 1.0f};/*clear color */

    path161=vgCreatePath(VG_PATH_FORMAT_STANDARD,VG_PATH_DATATYPE_F, 1.0000f,0.0000f, 0,0, (unsigned int)VG_PATH_CAPABILITY_APPEND_TO);
    vgAppendPathData(path161,sizeof(path161cmd)/sizeof(VGubyte),path161cmd,path161coord);

    if ( path161 == VG_INVALID_HANDLE )
        return CT_ERROR;
  ...
  matrix[6]=   3.200000e+001f; matrix[7]=   4.500000e+001f; matrix[8]=   1.000000e+000f;
  vgSeti(VG_MATRIX_MODE, VG_MATRIX_PATH_USER_TO_SURFACE);
  vgLoadMatrix(matrix);
  ...
  vgDrawPath(path161,VG_STROKE_PATH);
  
  vgDestroyPath(path161);
