...
CT_Result G30207_PathStroking_by_HYBRID(CT_File *AnsFile)
{
    VGPath path;
    VGPaint stroke;
    VGPath points;
    ...
    {
        VGfloat temp[6] = {0, 0.2f, 0.2f, 0.35f, 0.12f, 0.4f};
        for (i = 0; i < 6; i++)
        {
            coords[i] = temp[i] * WINDSIZEX * 2;
        }
    ...
    {
        VGfloat temp[8] = {0.12f, 0.4f, 0.04f, 0.45f, 0.35f, 0.1f, 0.1f, 0.1f};
        for (i = 0; i < 8; i++)
        {
            controlCoords[i] = temp[i] * WINDSIZEX * 2;
        }
    }
    vgModifyPathCoords(path, 1, 1, coords);
    vgModifyPathCoords(points, 2, 6, coords);
    vgModifyPathCoords(control, 0, 2, controlCoords);
