...
CT_Result G30307_PathStroking_by_ATI (CT_File *AnsFile)
{
    VGPaint fillpaint;
    VGPaint strokepaint;
 ...
 VGPath path249;
 VGubyte  path249cmd[]={ /* 2 segments */
   VG_MOVE_TO_ABS    , VG_LCWARC_TO_ABS  };
 VGfloat path249coord[]={ /* 7 coordinates */
    -2.000000e+001f,  -3.000000e+000f,   2.000000e+001f,   5.000000e+000f,   1.100000e+001f,  -2.000000e+001f,
     3.000000e+000f};

    VGfloat matrix[9];
    VGfloat defaultColor[] = {1.0f, 1.0f, 1.0f, 1.0f};/* clear color */                  

    path249=vgCreatePath(VG_PATH_FORMAT_STANDARD,VG_PATH_DATATYPE_F, 1.0000f,0.0000f, 0,0, (unsigned int)VG_PATH_CAPABILITY_APPEND_TO);
    vgAppendPathData(path249,sizeof(path249cmd)/sizeof(VGubyte),path249cmd,path249coord);


    if ( path249 == VG_INVALID_HANDLE )
        return CT_ERROR;
  ...
  vgDrawPath(path249,VG_FILL_PATH|VG_STROKE_PATH);

  vgDestroyPath(path249);
