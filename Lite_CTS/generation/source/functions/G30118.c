...
CT_Result G30118_PathStroking_by_ATI (CT_File *AnsFile)
{
    VGPaint fillpaint;
    VGPaint strokepaint;
 ...
 VGfloat path231coord[]={ /* 6 coordinates */
    ...
    path230=vgCreatePath(VG_PATH_FORMAT_STANDARD,VG_PATH_DATATYPE_F, 1.0000f,0.0000f, 0,0, (unsigned int)VG_PATH_CAPABILITY_APPEND_TO);
    vgAppendPathData(path230,sizeof(path230cmd)/sizeof(VGubyte),path230cmd,path230coord);
    path231=vgCreatePath(VG_PATH_FORMAT_STANDARD,VG_PATH_DATATYPE_F, 1.0000f,0.0000f, 0,0, (unsigned int)VG_PATH_CAPABILITY_APPEND_TO);
    vgAppendPathData(path231,sizeof(path231cmd)/sizeof(VGubyte),path231cmd,path231coord);

    if ( path230 == VG_INVALID_HANDLE )
        return CT_ERROR;
    if ( path231 == VG_INVALID_HANDLE )
        return CT_ERROR;
  ...
  vgDrawPath(path230,VG_FILL_PATH|VG_STROKE_PATH);
  ...
  vgSeti(VG_STROKE_CAP_STYLE,VG_CAP_SQUARE);
  vgSeti(VG_STROKE_JOIN_STYLE,VG_JOIN_BEVEL);
  vgDrawPath(path231,VG_FILL_PATH|VG_STROKE_PATH);

    vgDestroyPath(path230);
    vgDestroyPath(path231);
