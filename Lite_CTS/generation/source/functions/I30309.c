...
CT_Result I30309_PaintGradient_by_HYBRID(CT_File *AnsFile)
{
    VGPaint stroke;
    VGPaint fill;
    VGPath rect;
    ...
    VGfloat gradient[5]		= {33.25f, 33.25f, 33.45f, 33.45f, 0.3f};
    VGubyte rectCmds[5]		= {VG_MOVE_TO, VG_VLINE_TO_REL, VG_HLINE_TO_REL, VG_VLINE_TO_REL, VG_CLOSE_PATH};
    VGfloat rectCoords[5]	= {0.1f, 0.1f, 0.8f, 0.8f, -0.8f};
    VGint i;

	/* Tests the same things as linear_gradient but with gradient values out of
	 * bounds
	 */
	...
	for (i = 0; i < 5; i++)
    {
		rectCoords[i] = rectCoords[i] * WINDSIZEX;
    }
    ...
    vgScale(0.15f * WINDSIZEX, 0.15f * WINDSIZEX);
    vgTranslate(-0.25f * WINDSIZEX, -0.25f * WINDSIZEX);
