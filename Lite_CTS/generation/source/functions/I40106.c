 ...
 * Rotate both the path and the pattern image.
 *-------------------------------------------------------------------*/
CT_Result I40106_PaintPatternImage_by_HYBRID(CT_File *AnsFile)
{
    VGImage checker;
    VGPaint paint;
    VGPath	path;
	...
	/*---------------------------------------------------------------------
	 * Translate the image out of the visible area totally.
	 *-------------------------------------------------------------------*/
    vgSeti(VG_MATRIX_MODE, VG_MATRIX_FILL_PAINT_TO_USER);
    vgLoadIdentity();
	vgRotate(45);
