 ...
 * VG_PAINT_PATTERN_TILING_MODE = VG_TILE_PAD.
 *-------------------------------------------------------------------*/
CT_Result I40101_PaintPatternImage_by_HYBRID(CT_File *AnsFile)
{
    VGImage checker;
    VGPaint paint;
    VGPath	path;
	...
	/* Create the paint and set parameters for the pattern pad */
    paint = vgCreatePaint();
	...
	/*---------------------------------------------------------------------
	 * Scale rotate and translate the image by a reasonable amounts.
	 *-------------------------------------------------------------------*/
    vgSeti(VG_MATRIX_MODE, VG_MATRIX_FILL_PAINT_TO_USER);
    vgLoadIdentity();
    vgRotate(45);
