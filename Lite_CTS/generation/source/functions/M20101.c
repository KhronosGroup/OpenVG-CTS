...
CT_Result M20101_VguHLPrim_by_HUONE (CT_File *AnsFile)               
{
	...
	for ( i = VG_PATH_DATATYPE_S_8; i <= VG_PATH_DATATYPE_F; i++ ) {
		path  = vgCreatePath( VG_PATH_FORMAT_STANDARD, i, 1.0f, 0.0f, 0, 0, VG_PATH_CAPABILITY_ALL );
		if ( path == VG_INVALID_HANDLE ) {
			vgDestroyPaint(paint);
			return CT_ERROR;
		}

		vguLine(path, 1.25f, 1.25f, 62.75f, 32.75f);
