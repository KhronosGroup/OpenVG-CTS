 ...
 * VG_PAINT_PATTERN_TILING_MODE = VG_TILE_REPEAT.
 *-------------------------------------------------------------------*/
CT_Result I40103_PaintPatternImage_by_HYBRID(CT_File *AnsFile)
{
    VGImage checker;
    VGPaint paint;
    VGPath	path;
	...
	/* Create the paint and set parameters for the pattern repeat */
    paint = vgCreatePaint();
	...
	/*---------------------------------------------------------------------
	 * Rotate the image by 45 degrees and translate by large amounts.
	 *-------------------------------------------------------------------*/
    vgSeti(VG_MATRIX_MODE, VG_MATRIX_FILL_PAINT_TO_USER);
    vgLoadIdentity();
    vgRotate(45);
